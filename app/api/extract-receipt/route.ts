...
【🚀 쿠팡 주문내역 (최우선 규칙)】
이미지에 "로켓배송", "로켓직구", "로켓프레시", "로켓" 단어가 하나라도 보이면 쿠팡 주문으로 판단하고,
아래 4가지만 추출하세요. 다른 규칙보다 이 규칙이 우선합니다.

1. item: 상품명 (옵션·색상·사이즈·수량 표기는 제외, 핵심 상품명만)
2. amount: 해당 상품의 최종 결제 금액 (수량이 여러 개면 수량×단가 합산, 숫자만)
3. place: 반드시 "쿠팡" 고정
4. date: 날짜가 보이면 "MM월 DD일" 형식, 없으면 빈 문자열 ""

- method는 항상 "기타"
- 배송비·쿠팡캐시·적립금·할인·쿠폰·총결제금액 행은 반드시 제외
- 상품이 여러 개면 각 상품마다 별도 항목으로 분리
